export const NEXT_PROBE_KEY = '__DNDGEM_META_NEXT' as const;
...
export function ensureProbe(): CompatProbe {
  ...
  const holder = window as unknown as Record<typeof NEXT_PROBE_KEY, CompatProbe | undefined>;
  holder[NEXT_PROBE_KEY] ??= emptyProbe();
  return holder[NEXT_PROBE_KEY];
}
