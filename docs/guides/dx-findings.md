...
| DX-004 | Publication      | Public `npm install @dndgem/*` not available yet                        | External install path blocked until DND-2.5 | HIGH     | Documented current vs future install; DND-2.5 publish |
...
| DX-006 | A11y / keyboard  | Pointer-only validated drag                                             | Keyboard users lack a product path          | MEDIUM   | Deferred to DND-2.4 baseline                          |
