...
Deterministic Adaptive Auto-Layout (Phase 3) is defined as intent enrichment composed with existing `solveLayout` — not a second solver. Provenance (Source Intent vs Generated Placement vs Effective Solver Input) is mandatory. Previous layout remains stability-only (not an origin). See [auto-layout-contract.md](./auto-layout-contract.md), [auto-layout-engine.md](./auto-layout-engine.md), and [ADR-0014](../adr/ADR-0014-auto-layout-enrichment-provenance.md). Minimal public Core export: `createAutoLayoutProposal` (DND-3.4); DOM/React opt-in `autoLayout` (default off). Published `0.1.0-alpha.0` does not include Auto-Layout yet.
