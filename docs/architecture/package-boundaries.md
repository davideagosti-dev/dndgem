...
Current adapter: `@dndgem/react` (published). `@dndgem/vue` is **implemented in the repository** (DND-FX.2) and **not yet published**. Planned (do not stub): `@dndgem/angular`, `@dndgem/svelte`.
