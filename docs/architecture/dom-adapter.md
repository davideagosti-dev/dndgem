...
- Raw planner throw/reject → Phase 3 declaration-order Auto-Layout (deterministic-middle fallback requires `createOrchestratedLayoutPlanner` / app wrapper)
- Planner snapshot may observe committed `previous` as stability context; explicit replan omits Auto-Layout `previous` so advisory order competes fresh
- Last successful advisory order may be retained for idle/drop recomposes without re-invoking the planner (Core-normalized; never provenance)
...
First-party orchestration (`runLayoutPlanner`, full deterministic fallback chain) lives in private `@dndgem/intelligence`. Compose it at the application boundary when desired.
