...
const svelteExample = 'http://127.0.0.1:5178';

test.describe('svelte example integration', () => {
  test.use({ baseURL: svelteExample });

  test('svelte example applies geometry, Auto-Layout, and accepts a drag drop', async ({
    page,
  }) => {
    await page.goto('/');
    await expect(page.getByRole('heading', { name: 'DnDGem Svelte Example' })).toBeVisible();
    ...
    await expect
      .poll(async () => {
        return page.evaluate(() => {
          const probe = (window as unknown as { __DNDGEM_SVELTE?: SvelteProbe }).__DNDGEM_SVELTE;
  ...
  test('svelte example resize updates measured space from engine state', async ({ page }) => {
    await page.goto('/');
    await expect
      .poll(async () => {
        return page.evaluate(() => {
    ...
    const initialWidth = await page.evaluate(() => {
      return (
        (window as unknown as { __DNDGEM_SVELTE?: SvelteProbe }).__DNDGEM_SVELTE?.spaceWidth ?? 0
      );
    });
  ...
  test('svelte example Escape cancel restores committed layout', async ({ page }) => {
    await page.goto('/');
    const item = page.getByTestId('item-revenue');
    await expect(item).toHaveCSS('left', '12px');
    const box = await item.boundingBox();
    ...
    await expect
      .poll(async () => {
        return page.evaluate(() => {
          return (
            (window as unknown as { __DNDGEM_SVELTE?: SvelteProbe }).__DNDGEM_SVELTE?.cancelCount ??
            0
          );
        });
      })
  ...
  test('svelte example preserves consumer ARIA and tabindex', async ({ page }) => {
    await page.goto('/');
    const item = page.getByTestId('item-revenue');
    ...
    await expect
      .poll(async () => {
        return page.evaluate(() => {
          return (
            (window as unknown as { __DNDGEM_SVELTE?: SvelteProbe }).__DNDGEM_SVELTE?.spaceWidth ??
            0
          );
        });
      })
