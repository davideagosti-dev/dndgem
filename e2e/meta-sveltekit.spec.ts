...
const probeKey = '__DNDGEM_META_SVELTEKIT' as const;

test.describe('SvelteKit compatibility', () => {
  ...
  test('production server renders the board route without DnDGem geometry', async ({ request }) => {
    ...
    expect(html).toContain('DnDGem SvelteKit Compat');
  ...
  test('hydrates a client session with Auto-Layout, drag, resize, Escape, and ARIA', async ({
    ...
    await page.goto('/');
    await expect(page.getByRole('heading', { name: 'DnDGem SvelteKit Compat' })).toBeVisible();
