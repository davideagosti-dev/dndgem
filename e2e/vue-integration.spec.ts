...
const vueExample = 'http://127.0.0.1:5176';

test.describe('vue example integration', () => {
  test.use({ baseURL: vueExample });

  test('vue example applies geometry, Auto-Layout, and accepts a drag drop', async ({ page }) => {
    await page.goto('/');
    await expect(page.getByRole('heading', { name: 'DnDGem Vue Example' })).toBeVisible();
    ...
    await expect
      .poll(async () => {
        return page.evaluate(() => {
          const probe = (window as unknown as { __DNDGEM_VUE?: VueProbe }).__DNDGEM_VUE;
  ...
  test('vue example resize updates measured space from engine state', async ({ page }) => {
    await page.goto('/');
    await expect
      .poll(async () => {
        return page.evaluate(() => {
    ...
    const initialWidth = await page.evaluate(() => {
      return (window as unknown as { __DNDGEM_VUE?: VueProbe }).__DNDGEM_VUE?.spaceWidth ?? 0;
    });
  ...
  test('vue example Escape cancel restores committed layout', async ({ page }) => {
    await page.goto('/');
    const item = page.getByTestId('item-revenue');
    await expect(item).toHaveCSS('left', '12px');
    const box = await item.boundingBox();
    ...
    await expect
      .poll(async () => {
        return page.evaluate(() => {
          return (window as unknown as { __DNDGEM_VUE?: VueProbe }).__DNDGEM_VUE?.cancelCount ?? 0;
        });
      })
      .toBeGreaterThan(0);
  ...
  test('vue example preserves consumer ARIA and tabindex', async ({ page }) => {
    await page.goto('/');
    const item = page.getByTestId('item-revenue');
