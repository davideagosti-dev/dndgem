...
DnDGem is in **early public Alpha**. Phase 1 Technical MVP is **CLOSED**. Framework Expansion Gate is **COMPLETE** (DND-FX.1–FX.6). Current npm `@alpha` is **`0.1.0-alpha.4`**. Phase 4 is **COMPLETE AND SHIPPED** in `0.1.0-alpha.4` (DND-4.1–DND-4.5). Do not implement `@dndgem/next|nuxt|sveltekit` packages, public intelligence packages, model assistance, or Flutter without an authorized sprint.
